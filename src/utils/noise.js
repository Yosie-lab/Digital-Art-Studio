...
export class SimplexNoise {
  /**
   * @param {number} seed - 乱数シード (デフォルトはランダム)
   */
  constructor(seed = Math.random() * 2147483647) {
    const p = new Uint8Array(256);
    for (let i = 0; i < 256; i++) p[i] = i;

    // シード付き Fisher–Yates シャッフル
  ...
  /** 3Dシンプレックスノイズ (-1 ~ +1) — 時間変化のあるフィールドに使用 */
  noise3D(xin, yin, zin) {
...
/**
 * FBMノイズ (Fractal Brownian Motion)
 * 複数オクターブのノイズを重ねて自然な見た目を生成
 */
